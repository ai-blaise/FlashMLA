"""Full-NVFP4 V32 sparse-decode smoke test."""
...
kv = make_constant_full_nvfp4(num_blocks, page_block_size)
...
out, _ = run_kernel(q, kv, indices, topk_length, sm_scale)
of = out.float()
has_nan = torch.isnan(out).any().item()
has_inf = torch.isinf(out).any().item()
err = abs(of.mean().item() - 1.0)
print("Phase 2 smoke: all full-NVFP4 score dimensions dequantize to 1.0")
