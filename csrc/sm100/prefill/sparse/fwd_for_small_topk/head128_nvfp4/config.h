...
static constexpr bool IS_NVFP4_KV = true;
...
static constexpr int TMA_K_STRIDE_FOR_DECODING = D_NOPE/2 + 2*D_ROPE;  // NVFP4: nope packed at 0.5 bytes/element
// NVFP4 variant: block_size=16, fp8_e4m3 scales. D_NOPE/16 = 28 scales
// per token, padded to 32 for 16-byte SMEM alignment.
static constexpr int NUM_SCALES_EACH_TOKEN = 32;  // NVFP4 e4m3 scales (was 8 for FP8 e8m0)
...
struct SharedMemoryPlan {
    ...
    // NVFP4 variant: K_raw stores packed e2m1 nibbles, half the bytes of FP8.
    array_aligned<uint8_t, B_TOPK*(D_K/4)> K_raw[NUM_RAW_K_BUFS];
    ...
    // NVFP4: fp8_e4m3 scales, one per 16-element block.
    CUTE_ALIGNAS(16) fp8_e4m3 scales[NUM_INDEX_BUFS][B_TOPK][NUM_SCALES_EACH_TOKEN];
